...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (reviewForm && reviewsList) {
    ...
    async function loadServices() {
        try {
        const res = await fetch("../api/services/get_services.php");
        ...
        try {
            ...
            servicesGrid.innerHTML = `<div style="color:red;">Ошибка загрузки данных.<br>Ответ сервера:<br>${text}</div>`;
            return;
        }

        servicesGrid.innerHTML = "";
        if (!Array.isArray(services) || services.length === 0) {
            servicesGrid.innerHTML = `<div class="no-services">Пока нет услуг.</div>`;
            return;
        }
        ...
        servicesGrid.querySelectorAll(".edit-btn").forEach(btn => {
            ...
            // Включаем редактирование только для этой карточки
        ...
        servicesGrid.querySelectorAll(".cancel-btn").forEach(btn => {
            btn.addEventListener("click", () => loadServices());
        });
        ...
        servicesGrid.querySelectorAll(".update-btn").forEach(btn => {
            ...
            fetch("../api/services/update_service.php", { method: "POST", body: formData })
                .then(r => r.text())
                .then(result => {
                if (result.trim() === "success") loadServices();
                else alert("Ошибка при сохранении: " + result);
                });
            });
        });
        ...
        servicesGrid.innerHTML = `<div style="color:red;">Ошибка загрузки: ${err.message}</div>`;
        }
    }
